...
不同的底盘，不同的手臂（以转动为主，以伸缩为主），不同的gripper，模块化：装上这款手臂，就是电工，换上这对手臂，就是按摩工 （这将是一个巨大的市场，列一个表，2000种endeffector，理论上任何一种工具都可以是endeffector，直接把工具设计成手比设计一款能够使用工具的手容易的多，只需要装上接口，这是极致的实用性）
